  ...
  return {
    ...
    async load(id, options) {
      ...
      if (id === resolvedVirtualModuleId) {
        ...
        let code = `import {onRenderClient,onRenderHtml} from '${virtualAdapterId}';`
          + 'export const config={onRenderClient,onRenderHtml};'
          + 'export const routes=['
        ...
        // Always export errorRoute — null if _error/+Page.tsx doesn't exist
        // so that RouterApp can safely import it without checking for undefined
        if (errorRoute) {
          const e = errorRoute
      ...
      if (id === resolvedVirtualManifestId) {
        // If in DEV or building the Client, the manifest is not needed/does not exist yet.
        if (!isProd || !options?.ssr) return ''
        // In PROD (Build Server), read manifest.json generated by the client build
        // and transform it into a native JavaScript export
      ...
      if (id === resolvedVirtualEntryClientId) {
        return `import{onRenderClient}from'${virtualAdapterId}';onRenderClient();`
      }

      if (id === resolvedVirtualManifestId) {
        if (!isProd || !options?.ssr) return 'export default {}' // In dev restituisce oggetto vuoto
        const manifestPath = path.join(viteConfigRoot, '../dist/client/.vite/manifest.json')
        const manifestContent = fs.readFileSync(manifestPath, 'utf8')
        return `export default ${manifestContent}`
      }

      // NUOVO: Modulo di Setup
