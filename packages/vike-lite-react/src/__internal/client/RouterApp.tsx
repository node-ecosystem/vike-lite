...
import { buildNavigationPageContext, buildPageContextJsonUrl, consumeMatchingInitialContext, createLinkClickHandler, createLinkPrefetchHandler, createRoutePrefetcher, fetchPageContextJson, finalizeNavigation, loadViewModules, tryRecoverFromStaleModuleGraph, type ViewComponents } from 'vike-lite/__internal/client'
...
export function RouterApp(props: RouterProps) {
  ...
  useEffect(() => {
    ...
    const handleProgrammaticNavigate = (e: Event) => {
      ...
      if (detail.resolve) reloadResolvers.current.push(detail.resolve)
      setCurrentUrl(globalThis.location.href)
      setCurrentPathname(stripBase(globalThis.location.pathname))
    }
